...
class Request
{
    public function __construct(
        ...
        // Добавляем свойство для хранения тала зароса
        private string $body,
    ) {
    }
    ...
    // Метод для получение пути запроса
    // Напрмер, для http://example.com/some/page?x=1&y=acb
    // путем будет строка '/some/page'
    ...
    // Метод для получение значения
    // определенного параметра строки запроса
    // Напрмер, для http://example.com/some/page?x=1&y=acb
    ...
    public function query(string $param): string
    {
        ...
        if (empty($value)) {
            // Если нет значение параметра пусто - бросаем исключение
            throw new HttpException(
    ...
    public function header(string $header): string
    {
        ...
        if (empty($value)) {
            // Если нет значение заголовка пусто - бросаем исключение
